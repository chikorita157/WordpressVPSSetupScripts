...
fin.close()
fin = open(os.path.join("/etc/nginx", "nginx.conf"), "wt")
fin.write(data)
fin.close()
...
data = data.replace('<targetusername>', userName)
fin.close()
fin = open(os.path.join("/etc/php/7.4/fpm/pool.d/conf.d", "www.conf"), "wt")
fin.write(data)
fin.close()
...
fin.close()
fin = open(os.path.join("/etc/nginx/conf.d", domainName + ".conf"), "wt")
fin.write(data)
fin.close()
...
os.system("adduser --disabled-password --gecos " + userName)
